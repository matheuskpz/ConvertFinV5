...
def api_convert():
    ...
    if output_fmt == "ofx":
        ...
        headers = ["Data", "Descrição", "Valor (R$)", "Banco"]
        ...
        for i, r in enumerate(rows, 2):
            ws.cell(row=i, column=1, value=r["data"])
            ws.cell(row=i, column=2, value=r["descricao"])
            val = r["valor"]
            total += val
            cell_val = ws.cell(row=i, column=3, value=val)
            cell_val.number_format = '#,##0.00'
            if val < 0:
                cell_val.font = Font(color="DC2626")
            else:
                cell_val.font = Font(color="16A34A")
            ws.cell(row=i, column=4, value=r["banco"])

        # Linha de total
        total_row = len(rows) + 2
        ws.cell(row=total_row, column=1, value="TOTAL").font = Font(bold=True)
        tc = ws.cell(row=total_row, column=3, value=round(total, 2))
        ...
        writer.writerow(["Data", "Descrição", "Valor (R$)", "Banco"])
        total = 0.0
        for r in rows:
            val_fmt = f"{r['valor']:.2f}".replace(".", ",")
            writer.writerow([r["data"], r["descricao"], val_fmt, r["banco"]])
            total += r["valor"]
        writer.writerow(["TOTAL", "", f"{round(total,2):.2f}".replace(".", ","), ""])
        content  = ("\ufeff" + buf.getvalue()).encode("utf-8")  # BOM para Excel abrir certo
        mimetype = "text/csv"
...
# ──────────────────────────────────────────────
def startup():
    """Inicializa banco e cria admin padrão. Chamado na importação do módulo."""
    try:
        init_db()
        db = sqlite3.connect(DB_PATH)
        db.row_factory = sqlite3.Row
        if not db.execute("SELECT id FROM users WHERE is_admin=1").fetchone():
            ...
            db.commit()
            print("[ConvertFin] Admin criado: admin@convertfin.com.br / admin123")
        db.close()
        print("[ConvertFin] Startup OK")
    except Exception as e:
        print(f"[ConvertFin] Erro no startup: {e}")

# Executa ao importar (funciona com gunicorn e com python app.py)
startup()
